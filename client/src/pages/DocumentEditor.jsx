...
const DocumentEditor = () => {
  ...
  // Auto-collapse split mode on mobile screens
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <div className="h-[calc(100vh-140px)] flex flex-col bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between px-4 sm:px-6 py-4 border-b border-slate-100 bg-slate-50/50 gap-4 flex-shrink-0">
        <div className="flex items-center gap-3 w-full md:w-auto min-w-0">
          <button 
            onClick={() => navigate('/')}
            className="p-2.5 bg-white text-slate-500 hover:text-slate-700 hover:bg-slate-100 rounded-xl border border-slate-200 shadow-sm transition-all flex-shrink-0"
          >
          ...
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            {id === 'new' ? (
              <input 
                ...
                className="bg-white border border-slate-200 rounded-xl px-3 py-1.5 outline-none text-sm font-bold text-slate-800 focus:border-indigo-500 w-full max-w-[180px] xs:max-w-[240px] sm:max-w-xs shadow-sm"
              />
            ) : (
        ...
        <div className="flex flex-wrap items-center justify-between md:justify-end gap-3 w-full md:w-auto flex-shrink-0">
          <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200/50 shadow-inner">
            ...
            <button 
              ...
              className={`hidden md:flex p-2 rounded-lg text-xs font-bold transition-all items-center gap-1.5 ${
          ...
          <button 
            onClick={handleSave}
            disabled={saving}
            className={`px-4 sm:px-5 py-2.5 rounded-xl text-sm font-semibold flex items-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] shadow-lg ${
              saveSuccess 
      ...
      <div className="bg-slate-50 border-t border-slate-100 px-4 sm:px-6 py-2.5 flex flex-col sm:flex-row sm:items-center justify-between text-[11px] text-slate-400 font-semibold gap-2 flex-shrink-0">
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <span className="flex items-center gap-1 mr-1 text-indigo-500"><Info className="w-3.5 h-3.5" /> Markdown Supported</span>
          <span className="bg-white border border-slate-200/60 px-1.5 py-0.5 rounded"># H1</span>
          <span className="bg-white border border-slate-200/60 px-1.5 py-0.5 rounded">## H2</span>
          <span className="bg-white border border-slate-200/60 px-1.5 py-0.5 rounded">**Bold**</span>
          <span className="bg-white border border-slate-200/60 px-1.5 py-0.5 rounded">*Italic*</span>
          <span className="bg-white border border-slate-200/60 px-1.5 py-0.5 rounded">`Code`</span>
          <span className="bg-white border border-slate-200/60 px-1.5 py-0.5 rounded">- Bullet</span>
        </div>
