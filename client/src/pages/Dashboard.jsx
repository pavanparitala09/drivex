...
import { getFiles, setCurrentFolder, deleteFile, toggleStarFile } from '../store/fileSlice';
import { getFolders, deleteFolder, updateFolder, toggleStarFolder } from '../store/folderSlice';
import { File, Folder, Image, FileText, Film, ChevronRight, Trash2, Edit, Share2, Info, Star, Palette, Tag, Sparkles, MoreVertical } from 'lucide-react';
  ...
  const handleTagToggle = async (id, type, name, color) => {
    try {
      if (type === 'file') {
        ...
        dispatch(getFolders({ parentFolder: currentFolder }));
      }
      setContextMenu(null);
    } catch (err) {
      console.error(err);
      ...
      <div className="flex items-center text-xl font-semibold text-slate-800 mb-8 px-2">
        {breadcrumbs.map((crumb, index) => (
          <React.Fragment key={`${crumb.id || 'root'}-${index}`}>
            <span 
              className={`cursor-pointer hover:bg-slate-100 px-3 py-1.5 rounded-xl transition-colors ${index === breadcrumbs.length - 1 ? 'text-indigo-600 bg-indigo-50' : 'text-slate-500'}`}
              onClick={() => handleBreadcrumbClick(crumb, index)}
            >
              {crumb.name}
            </span>
            {index < breadcrumbs.length - 1 && <ChevronRight className="w-5 h-5 mx-1 text-slate-300" />}
          </React.Fragment>
        ))}
      </div>
      ...
      <div className="mb-8 px-2 flex-1">
        ...
        {files.length > 0 ? (
          ...
          <div className="text-center py-20 bg-slate-50/50 border border-dashed border-slate-200 rounded-3xl">
            <p className="text-slate-400 font-medium text-lg">No files in this folder.</p>
            <p className="text-slate-400 text-sm mt-2">Upload a file or create a new folder to get started.</p>
          </div>
        )}
      </div>
